...
def CommentView(request):
    ...
    site_date = datetime.datetime.strptime('2018-04-12','%Y-%m-%d')
    context = {}
    context['site_date'] = site_date
    context['hello'] = "Hello World"

    messages = get_messages_from_csv()
...
def addComment(request):
    ...
    stu1 = [user_name, message, time_str]

    write_message_to_csv(stu1)

    messages = get_messages_from_csv()

    context = {}
    context['message'] = "添加留言成功"
    context['contexts'] = messages
    print(messages)
    return render(request, 'blog/comment.html', context=context)


def get_messages_from_csv():

    f = open('data/message.cvs', 'r')
    csvreader = csv.reader(f)
    csvreader = list(csvreader)

    result = []
    if csvreader == None or len(csvreader) == 0:
        return result

    one_line = {}
    one_line['name'] = '欢迎加入地图数据爬取交流群'
    one_line['time'] = '2019-09-17 00:00:0'
...
def write_message_to_csv(data):
    ...
    '''
    # 打开文件，写模式为追加'a'
    out = open('data/message.cvs', 'a', newline='')
    # 设定写入模式
